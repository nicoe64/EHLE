...
BOOST_AUTO_TEST_CASE(privatTest) {
    // Test case 1: Same starting and ending points, with height 10.0
    float distance1 = Vertical::distance(0.0, 1.0, 0.0, 1.0, 10.0);
    std::cout << "Distance 1: " << distance1 << std::endl; // Expected: 20.0
    BOOST_CHECK_CLOSE(distance1, 20.0, 0.001);

    // Test case 2: Different starting and ending points, with height 0.0
    float distance2 = Vertical::distance(1.0, -1.0, -10.5, -20.11, 0.0);
    std::cout << "Distance 2: " << distance2 << std::endl; // Expected: 22.3034
    BOOST_CHECK_CLOSE(distance2, 22.3034, 0.001);

    // Test case 3: Different starting and ending points, with height 8.0
    float distance3 = Vertical::distance(1.0, -1.0, -10.5, -20.11, 8.0);
    std::cout << "Distance 3: " << distance3 << std::endl; // Expected: 38.3034
    BOOST_CHECK_CLOSE(distance3, 38.3034, 0.001);
}
